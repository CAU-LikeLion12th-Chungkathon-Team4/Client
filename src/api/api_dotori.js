...
export const dotoriCollectionOpen = async (dotori_collection_id) => {
    try {
        const response = await axios.get(`${baseUrl}/${dotori_collection_id}/open`);
        return response;
      } catch (error) {
        console.error(error);
        throw error;
...
export const dotoriDelete = async (dotori_id) => {
  try {
    const accessToken = localStorage.getItme("accessToken")
...
// 도토리 사진 반환
export const dotoriPictureList = async (dotori_id) => {
  try {
      const response = await axios.get(`${baseUrl}/dotori/delete/${dotori_id}`);
      return response;
    } catch (error) {
      console.error(error);
      throw error;
